...
//Determines whether we want to apply the density factor to the measurement.
//g and oz do not need to apply the density factor with each other, and same with cup/tsp/tbsp
//But g/oz need to divide by density factor with cup/tsp/tbsp, or multiply if vice versa
function DoApplyDensityFactor(recipeMeasurement, targetMeasurement) {
    if (recipeMeasurement === 'g' || recipeMeasurement === 'oz') {
        ...
        if (targetMeasurement === 'g' || targetMeasurement === 'oz') {
            return 'Multiply';
        }
    }
    return 'None';
...
//Takes the given ingredient object {amount, measurement, itemType} and converts its amount value as needed
//itemType is used to more accurately convert values based on the density factor of the recipe
export function ConvertIngredientData(ingredient) {
    ...
    var densityFactorApplication = DoApplyDensityFactor(ingredient.measurement, preferredIngredientType);
    if (densityFactorApplication === 'Multiply') {
        convertedIngredientAmount = convertedIngredientAmount * GetDensityFactor(ingredient.itemType);
    }
    else if (densityFactorApplication === 'Divide') {
        convertedIngredientAmount = convertedIngredientAmount / GetDensityFactor(ingredient.itemType);
    }
